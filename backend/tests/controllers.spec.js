const { signup, login, verifyEmail, logout }  = require('../src/controllers/authController');
const { chatHandler } = require('../src/controllers/chatController');
const { assignPets, listMatches }              = require('../src/controllers/matchController');
const { listPets, createPet }                  = require('../src/controllers/petController');
const { recordSwipe, listMySwipes }            = require('../src/controllers/swipeController');
const { getProfile, uploadAvatarHandler }      = require('../src/controllers/userController');
...
describe('Controller sanity suite (JS)', () => {
  ...
  test('signup → 201 & token cookie', async () => {
    ...
    const req = { body: { email: 'x@y.z', password: 'pw123' }, secure: false, headers: {} };
    const res = resMock();
